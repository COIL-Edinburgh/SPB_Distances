...
/*
 * ImageJ Plugin to measure single dots in 2 colour (blue and green) images. The user 
 * assigns which channel is which so although it merges the Blue and Green images, any 2 colour 
 * image can be used just assign the channels appropriately.
...
public class Dot_Distance implements PlugIn, MouseListener{
	...
	public void run(String arg) {
		...
		FileInfo theDirectory = imp.getOriginalFileInfo(); //Get the directory
		theparentDirectory = theDirectory.directory;
		
		//USER SELECTS THE COLOUR CHANNELS
	...
	public void MergeImages() {
		//Merge the Blue and Green Channels
	 ...
	 public void MeasureSPB(double xVal){
		  ...
		  * mouse clicks until the user selects "n"
	...
	public void outputinfo(double distance, int xID, int yID,int zPos, int Counter){
		/*
		 * Outputs the data in the form of a text file
		 * for import into Excel, R or Graphpad etc.
		 * Changing the .txt extension to .csv will
		 * allow Excel to automatically open the file
		 */
	...
	public void setImageNumbers(int Counter){
		...
		/*
		 * Mark each counted pair of SPB's with
		 * its corresponding number in the results
		 * file
		 */
